...
/******************************************************************************
function: Antialiasing RGB565 color scaling
parameter:
    color : RGB565 color value
    a     : Alpha value (0-255)
return: Scaled RGB565 color
******************************************************************************/
static inline uint16_t scale_rgb565(uint16_t color, uint8_t a)
{
    ...
    for(j = 0; j < H_Image; j++){
        for(i = 0; i < W_Image; i++){
            if(xStart+i < Paint.WidthMemory  &&  yStart+j < Paint.HeightMemory)//Exceeded part does not display
                Paint_SetPixel(xStart + i, yStart + j, (*(image + j*W_Image*2 + i*2))<<8 | (*(image + j*W_Image*2 + i*2+1)));
...
/******************************************************************************
function: Display a single character using Font_XL
parameter:
    Xstart : X coordinate
    Ystart : Y coordinate
    AsciiChar : ASCII character to display (space to tilde, but only space-Z supported)
    Color_Foreground : Color for the font (alpha weighted against white background)
******************************************************************************/
void Paint_DrawChar_XL(uint16_t Xstart, uint16_t Ystart, const char AsciiChar, uint16_t Color_Foreground)
{
    // Font_XL contains characters from ASCII 32 (space) to 32+26 (via indices 0-26)
    int char_index = (int)AsciiChar - FONT_XL_FIRST;
    
    if (char_index < 0 || char_index >= FONT_XL_COUNT) {
        return; // Character not supported
    }
    
    // Draw the character pixel by pixel with antialiasing
    for (uint16_t y = 0; y < FONT_XL_H; y++) {
        for (uint16_t x = 0; x < FONT_XL_W; x++) {
            ...
            if (alpha > 0) {
                ...
                Paint_SetPixel(Xstart + x, Ystart + y, pixel);
            }
        }
    }
}
...
/******************************************************************************
function: Display a string using Font_XL
parameter:
    Xstart : X coordinate
    Ystart : Y coordinate
    pString : String to display
    Color_Foreground : Color for the font
******************************************************************************/
void Paint_DrawString_XL(uint16_t Xstart, uint16_t Ystart, const char * pString, uint16_t Color_Foreground)
{
    ...
    if (Xstart > Paint.Width || Ystart > Paint.Height) {
        Debug("Paint_DrawString_XL Input exceeds the normal display range\r\n");
        return;
    }
    
    while (* pString != '\0') {
        // If X direction filled, reposition
        if ((Xpoint + FONT_XL_W) > Paint.Width) {
            Xpoint = Xstart;
            Ypoint += FONT_XL_H;
        }
        
        // If Y direction filled, wrap around
        if ((Ypoint + FONT_XL_H) > Paint.Height) {
            Xpoint = Xstart;
            Ypoint = Ystart;
        }
        
        Paint_DrawChar_XL(Xpoint, Ypoint, *pString, Color_Foreground);
        Xpoint += FONT_XL_W;
        pString++;
    }
}
